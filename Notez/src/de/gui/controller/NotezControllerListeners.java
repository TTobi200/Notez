...
public class NotezControllerListeners extends NotezControllerListenerBase<NotezController>
{
	...
	protected void initSettings()
	{
		...
		c.colUsername.setCellValueFactory(new PropertyValueFactory<NotezRemoteUser, String>(
				"username"));
		c.colShare.setCellValueFactory(new PropertyValueFactory<NotezRemoteUser, String>("share"));
	}

	protected void initProperties()
	{
		c.txtPropNotezWorkFold.setText(NotezProperties.get(NotezProperties.PROP_NOTEZ_WORK_FOLDER));
		c.txtPropNotezRemoteFold.setText(NotezProperties
				.get(NotezProperties.PROP_NOTEZ_REMOTE_FOLDER));

		c.cbStartRecOnStartup.setSelected(Boolean.valueOf(NotezProperties
				.get(NotezProperties.PROP_START_RECEIVER)));
		c.cbStartRecKeepRun.setSelected(Boolean.valueOf(NotezProperties
				.get(NotezProperties.PROP_LET_RECEIVER_RUNNING)));

		c.cbAlwaysAskToSave.setSelected(Boolean.valueOf(NotezProperties
				.get(NotezProperties.PROP_ALWAYS_SAVE_ON_EXIT)));

		c.cbAddNotez.selectedProperty().bindBidirectional(c.btnAdd.visibleProperty());
		c.cbPinNotez.selectedProperty().bindBidirectional(c.btnPin.visibleProperty());
		c.cbRemoveNotez.selectedProperty().bindBidirectional(c.btnDelete.visibleProperty());
		c.cbSaveNotez.selectedProperty().bindBidirectional(c.btnSave.visibleProperty());
		c.cbShareNotez.selectedProperty().bindBidirectional(c.btnShare.visibleProperty());
		c.cbGroupNotez.selectedProperty().bindBidirectional(c.pickNote.visibleProperty());
		c.cbPrintNotez.selectedProperty().bindBidirectional(c.btnPrint.visibleProperty());

		c.cbPinNotez.setSelected(Boolean.valueOf(NotezProperties
				.get(NotezProperties.PROP_BTN_PIN_VISIBLE)));
		c.cbGroupNotez.setSelected(Boolean.valueOf(NotezProperties
				.get(NotezProperties.PROP_BTN_GROUP_VISIBLE)));
		c.cbShareNotez.setSelected(Boolean.valueOf(NotezProperties
				.get(NotezProperties.PROP_BTN_SHARE_VISIBLE)));
		c.cbAddNotez.setSelected(Boolean.valueOf(NotezProperties
				.get(NotezProperties.PROP_BTN_ADD_VISIBLE)));
		c.cbSaveNotez.setSelected(Boolean.valueOf(NotezProperties
				.get(NotezProperties.PROP_BTN_SAVE_VISIBLE)));
		c.cbRemoveNotez.setSelected(Boolean.valueOf(NotezProperties
				.get(NotezProperties.PROP_BTN_REMOVE_VISIBLE)));
		c.cbPrintNotez.setSelected(Boolean.valueOf(NotezProperties
				.get(NotezProperties.PROP_BTN_PRINT_VISIBLE)));
	}

	@Override
	protected void initPinNote()
	{
		c.btnPin.selectedProperty().addListener(
			(s, o, n) -> {
	...
	{
		c.data.getPageData().textProperty().bindBidirectional(c.txtNote.textProperty());
		c.lblPage.textProperty()
				.bind(
					Bindings.concat(c.data.getPageData().curPageIndexProperty().add(1), " / ",
						c.data.getPageData().sizeProperty()));
		c.btnPrevPage.disableProperty().bind(c.data.getPageData().curPageIndexProperty().isEqualTo(0));
	...
	protected void addFileLink(final Hyperlink link, File note)
	{
		...
		link.setOnAction(e -> {
			...
			catch(Exception e1)
			{
				try
				{
					NotezDialog.showExceptionDialog(c.stage, "Error while opening folder",
