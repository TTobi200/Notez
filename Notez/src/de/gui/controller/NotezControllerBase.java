...
public abstract class NotezControllerBase<L extends NotezControllerListenerBase<?>> implements
		NotezFXMLInitializable
{
	...
	/**
	 * boolean binding indicating whether the text in {@link #txtNote} differs from the saved on
	 */
	...
	/**
	 * Method to close this notez. User gets asked if he likes to save the notez.
	 *
	 * @throws Exception
		...
		if (noteChanged.get() && askToSave)
		{
			...
			else
			{
				switch(NotezDialog.showRememberQuestionDialog(stage, "Save Changes",
					"Do you like to save the changes?", NotezProperties.PROP_ALWAYS_SAVE_ON_EXIT,
					true))
				{
					case CANCEL:
					case CLOSE:
	...
	{
		return NotezFileUtil.canBeUsedAsFilename(notezName) ? new File(NotezProperties.get(
			NotezProperties.PROP_NOTEZ_WORK_FOLDER, NotezFrame.DEF_LOCAL_NOTEZ_FOLDER)
				+ notezName
	...
	/**
	 * Get the text of the current note. if the note consists of several pages, behaviour is
	 * undefined.<br>
