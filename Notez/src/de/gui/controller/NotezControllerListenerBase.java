...
import de.util.FXMLInitializable;
import de.util.NotezSettings;
import de.util.NotezSettings.Setting;

public abstract class NotezControllerListenerBase<C extends NotezControllerBase<?, ?>> implements
		FXMLInitializable
{
	public static DataFormat NOTEZ_CONTROLLER_DATA_FORMAT = new DataFormat(
			NotezController.class.getName(), NotezControllerListeners.class.getName());
	...
	protected void initNoteChanged()
	{
		...
		c.noteChanged = c.data.textChangedProperty()
				...
				.or(stageSize.widthProperty().isEqualTo(c.getStage().widthProperty(), 0.2))
				.or(stageSize.heightProperty().isEqualTo(c.getStage().heightProperty(), 0.2));
	}
	...
	protected void setAsDndTarget(final Node node)
	{
		node.setOnDragOver(event -> {
			...
			if (event.getDragboard().hasContent(NOTEZ_CONTROLLER_DATA_FORMAT)
					&& !event.getDragboard().getContent(NOTEZ_CONTROLLER_DATA_FORMAT).equals(c.idx))
			{
				// Accept the move
	...
	protected void moveStageAnimatedTo(Point2D pos)
	{
		...
		new Timeline(new KeyFrame(DUR, new KeyValue(x, pos.getX()), new KeyValue(y, pos.getY())))
				.play();
	}
	...
	protected abstract void initSettings(Map<String, Setting<Object>> all);
}
